...
%\VignetteIndexEntry{The npRmpi Package}
%\VignetteDepends{npRmpi,boot,MASS}
...
\title{The \pkg{npRmpi} Package}

\Plainauthor{Tristen Hayfield, Jeffrey S.~Racine}

\Plaintitle{The npRmpi Package}

%\Shorttitle{The np Package}

\Abstract{ 
  
  We describe the \proglang{R} \pkg{npRmpi} package via a series of
  applications that may be of interest to applied econometricians.
  This vignette is based on \citet{v27i01:Hayfield+Racine:2008}.  The
  \pkg{npRmpi} package implements a variety of nonparametric and
  ...
  models, among others. The \pkg{npRmpi} package focuses on kernel methods
...
The \pkg{npRmpi} package implements recently developed kernel methods that
...
philosophy underlying \pkg{npRmpi} aims to provide an intuitive, flexible,
...
\pkg{npRmpi} is available from the Comprehensive \proglang{R} Archive
Network at \url{http://CRAN.R-project.org/package=np}.

Currently, a range of methods can be found in the \pkg{npRmpi} package
...
\citeauthor{RACINE_LIU:2007} \citeyear{RACINE_LIU:2007}, index models
...
\citeauthor{LI_RACINE:2007b} \citeyear{LI_RACINE:2007b},
\citeauthor{RACINE_OUYANG_LI:2007} \citeyear{RACINE_OUYANG_LI:2007}),
among others. The various functions in the \pkg{npRmpi} package are
...
\citet{ROBINSON:1988}, \citet{RACINE_LIU:2007} \\
\code{npplregbw} & Semiparametric Partially Linear  Regression Bandwidth Selection & \citet{ROBINSON:1988}, \citet{RACINE_LIU:2007} \\
...
\code{npscoef} & Semiparametric Smooth Coefficient  Regression& \citet{LI_RACINE:2007b}\\
\code{npscoefbw} & Semiparametric Smooth Coefficient  Regression Bandwidth Selection& \citet{LI_RACINE:2007b}\\
...
\code{uocquantile} & Compute Quantiles/Modes for Unordered, Ordered, and Numeric Data& \\
  ...
  \caption{\textbf{\pkg{npRmpi} functions.}}
\end{table}
	
In this article, we illustrate the use of the \pkg{npRmpi} package via a
...
implementation details of the \pkg{npRmpi} package in
...
users navigate the methods that reside in the \pkg{npRmpi} package. We
...
taken lightly. \pkg{npRmpi} is designed from the ground up to make
...
selection and the \pkg{npRmpi} package.

\subsection[Interacting with np functions]{Interacting with \pkg{npRmpi} functions}
...
\pkg{npRmpi} package contains a number of datasets whose variables have
...
\pkg{npRmpi} package to {\em automatically} use appropriate weighting
...
of an \pkg{npRmpi} function, however, it is preferable to begin with a
data frame having appropriately classed elements.

There are two ways in which you can interact with functions in
\pkg{npRmpi}, namely using data frames or using a formula interface, where
appropriate.  Every function in \pkg{npRmpi} supports both interfaces,
where appropriate.
...
frame to the appropriate \pkg{npRmpi} function, say
\begin{verbatim} 
...
We have tried to make \pkg{npRmpi} flexible enough to be of use to a wide
...
unadjusted $R^2$ of 23.1\%.  Next, we consider the local linear
nonparametric method proposed by \citet{LI_RACINE:2004} employing
cross-validated bandwidth selection using the method of
\citet{HURVICH_SIMONOFF_TSAI:1998}. Note that in this example we
conduct bandwidth selection and estimation in one step.
...
32.5\%.\footnote{See Section~\ref{r-squared section} for details on
  computation of the nonparametric $R^2$ measure.}
...
\begin{figure}[!ht]
\begin{center}
<<fig=TRUE,  multifig=TRUE, eval=TRUE, echo=FALSE, keep.source=TRUE>>=
...
\pkg{npRmpi} to leverage the \pkg{boot} package
...
model:\footnote{We would like to thank Jeffrey Wooldridge for allowing
  us to incorporate his data in the \pkg{npRmpi} package. Also, we would
...
The predicted square error on the hold-out data was $0.147$ for the
parametric linear model, $0.145$ for the traditional nonparametric
estimator that splits the data into subsets, and $0.134$ for the
nonparametric estimator that uses the full sample but smooths both the
qualitative and quantitative data. The nonparametric model that
smooths both the quantitative and qualitative data is more than 8\%
...
predicting 161/189 birthweights compared with 144/189 for the
parametric model.
...
geyser data and compute the density and distribution functions. To
plot the conditional distribution we use \code{cdf = TRUE} in
\code{plot}. Results are presented in Figure~\ref{faithful plot}. Note
...
\begin{figure}[!ht]
\begin{center}
<<fig=TRUE, png=TRUE, multifig=TRUE, eval=TRUE, echo=FALSE, keep.source=TRUE, height=4, width=8>>=
...
we compute the \code{npcdens} object.  To plot the conditional
distribution we use \code{cdf = TRUE} in \code{plot}.  Note that in this
...
Note that for this example we first call \code{npcdensbw} to avoid
...
as that found in \citet{LI_RACINE:2007b} and
\citet{RACINE_OUYANG_LI:2007}. We compare a simple linear model with
...
\proglang{MPI}-aware version of the \pkg{npRmpi} package that uses some of
...
computing platforms).  The functionality of \pkg{npRmpi} and \pkg{npRmpi}
...
The \pkg{npRmpi} package offers users of \proglang{R} a variety of
...
the functionality of the \pkg{npRmpi} package via a series of illustrative
applications that may be of interest to applied econometricians
...
The help files accompanying many functions found in the \pkg{npRmpi}
...
\pkg{npRmpi} package with appropriate acknowledgment of course.
...
\pkg{npRmpi} package and to the exposition of this article. Racine would
