...
\Plaintitle{Entropy-Based Inference using R and the npRmpi Package: A
  Primer}
...
The function \code{npsymtest} computes the nonparametric metric entropy
(normalized Hellinger of \citet{GRANGER_MAASOUMI_RACINE:2004})
...
data, $f(y)$ and $f(\tilde y)$, respectively. $Y$ must be univariate
and can be a time series, continuous, or even categorical valued so
long as the outcomes are not character strings.
